...
fn cancel_bfs_replay_and_phase1_invariants() {
    ...
    for ev in &events {
        ...
        if ev.kind == KIND_TASK_JOINED_U64 {
            ...
            assert!(
                started.contains(&joined),
                "joined unknown task id {}",
                joined
            );
            assert!(
    ...
    for tid in started {
        assert!(
            finished.contains(&tid),
            "started task {} did not finish",
            tid
        );
    }
    ...
    assert!(
        ...
        "expected at least one TaskCancelled"
    );
    assert_eq!(
        cancel_order[0], 0,
        "BFS cancel traversal must begin at root 0"
    );
...
fn negative_join_unknown_task_rejected() {
    ...
    assert!(
        ...
        "expected replay failure for unknown join"
    );
...
fn negative_double_join_rejected() {
    ...
    assert!(
        !replay.status.success(),
        "expected replay failure for double join"
    );
...
fn negative_cancel_unknown_root_rejected() {
    ...
    assert!(
        !replay.status.success(),
        "expected replay failure for unknown cancel root"
    );
...
"#;
    ...
    let spawned: Vec<(u64, u64)> = events
        ...
        .map(|e| {
            (
                e.parent.expect("spawn parent"),
                e.child.expect("spawn child"),
            )
        })
        .collect();
...
"#;
    ...
    if let Some(first_cancel_idx) = events
        .iter()
        .position(|e| e.kind == KIND_TASK_CANCELLED_U64)
    {
        assert!(
    ...
    if !cancel_order.is_empty() {
        ...
        assert_eq!(
            cancel_order, sorted,
            "cancel order must remain deterministic"
        );
    }
}
...
fn jsonl_is_overwritten_each_run_not_appended() {
    ...
    assert_eq!(
        events[0].seq, 0,
        "first event seq should reset to 0 per run"
    );
}
