...
use crate::ast::{BinOp, Capability, Expr, Function, Item, Literal, Program, Stmt, Type, NetPortSpec};
...
// AUTO-GENERATED by NEX codegen. DO NOT EDIT.",
    ...
    w.l("// - Step 0.2.5: Panic-safe root cleanup");
    w.l("// - Step 0.2.6: Registry pruning (no unbounded growth)");
    w.l("// v0.3.0: Advanced Capability Policies");
    w.l("// - Step 0.3.1: fs.read globs + path normalization (anti-traversal)");
    w.l("// - Step 0.3.3: net.listen port ranges (8000..9000)");
    w.l("");
    ...
    w.l("    // __NEX_FS_READ_ALLOW: &[&str]");
    w.l("    let norm_path = match __nex_normalize_rel_path(path) {");
    w.l("        Some(p) => p,");
    ...
    w.l("            None => continue,");
    w.l("        };");
    w.l("        if __nex_glob_match(&norm_glob, &norm_path) {");
    w.l("            return true;");
    w.l("        }");
    w.l("    }");
    w.l("    false");
    ...
    // ------------------------------
    // v0.3.1 helpers: path normalization + glob matching
    // ------------------------------
    w.l("fn __nex_normalize_rel_path(raw: &str) -> Option<String> {");
    w.ind += 1;
    w.l("// Normalize a user-provided relative path string.");
    w.l("// Security rule: disallow escaping above root (.. when stack is empty).");
    w.l("// Also normalize Windows separators to '/' so policies are stable.");
    w.l("let s = raw.replace('\\\\', \"/\");");
    w.l("let mut stack: Vec<&str> = Vec::new();");
    w.l("for part in s.split('/') {");
    w.ind += 1;
    w.l("if part.is_empty() || part == \".\" { continue; }");
    w.l("if part == \"..\" {");
    w.ind += 1;
    w.l("if stack.is_empty() { return None; }");
    w.l("stack.pop();");
    w.l("continue;");
    w.ind = w.ind.saturating_sub(1);
    w.l("}");
    ...
    w.l("fn __nex_glob_match(glob: &str, s: &str) -> bool {");
    w.ind += 1;
    w.l("if glob == \"*\" { return true; }");
    w.l("let parts: Vec<&str> = glob.split('*').collect();");
    w.l("if parts.len() == 1 { return glob == s; }");
    w.l("let mut pos: usize = 0;");
    w.l("for (i, p) in parts.iter().enumerate() {");
    w.ind += 1;
    w.l("if p.is_empty() { continue; }");
    w.l("match s[pos..].find(p) {");
    w.ind += 1;
    w.l("Some(idx) => {");
    w.ind += 1;
    w.l("if i == 0 && !glob.starts_with('*') && idx != 0 { return false; }");
    w.l("pos += idx + p.len();");
    w.ind = w.ind.saturating_sub(1);
    w.l("}");
    w.l("None => return false,");
    ...
    w.ind = w.ind.saturating_sub(1);
    w.l("}");
    w.l("if !glob.ends_with('*') {");
    w.ind += 1;
    w.l("if let Some(last) = parts.last() { return s.ends_with(last); }");
    w.ind = w.ind.saturating_sub(1);
    w.l("}");
    w.l("true");
    w.ind = w.ind.saturating_sub(1);
    ...
    w.l("fn __nex_fs_read_allowed(raw: &str) -> bool {");
    w.ind += 1;
    w.l("let norm = match __nex_normalize_rel_path(raw) {");
    w.ind += 1;
    w.l("Some(p) => p,");
    w.l("None => return false,");
    w.ind = w.ind.saturating_sub(1);
    w.l("};");
    w.l("__NEX_FS_READ_ALLOW.iter().any(|g| __nex_glob_match(g, &norm))");
    w.ind = w.ind.saturating_sub(1);
    w.l("}");
    w.l("");

    // v0.3.3: allow net.listen if in single list OR any range
    w.l("fn __nex_net_listen_allowed(port: i64) -> bool {");
    w.ind += 1;
    w.l("let single_ok = __NEX_NET_LISTEN_ALLOW.iter().any(|p| *p == port);");
    w.l("let range_ok = __NEX_NET_LISTEN_RANGE_ALLOW.iter().any(|(a,b)| port >= *a && port <= *b);");
    ...
    w.ind += 1;
    w.l("__NEX_CURRENT_TASK_ID.with(|c| *c.borrow())");
    w.ind = w.ind.saturating_sub(1);
    w.l("}");
    w.l("");
    ...
    w.ind += 1;
    w.l("__NEX_CURRENT_TASK_ID.with(|c| *c.borrow_mut() = id);");
    w.ind = w.ind.saturating_sub(1);
    w.l("}");
    w.l("");

    w.l("fn __nex_get_current_token() -> __NexCancelToken {");
    w.ind += 1;
    w.l("__NEX_CURRENT_TOKEN.with(|t| t.borrow().clone())");
    w.ind = w.ind.saturating_sub(1);
    w.l("}");
    w.l("");
    ...
    w.l("fn __nex_alloc_task_id() -> __NexTaskId {");
    w.ind += 1;
    w.l("__NEX_TASK_ID_GEN.fetch_add(1, Ordering::SeqCst)");
    w.ind = w.ind.saturating_sub(1);
    w.l("}");
    w.l("");
    ...
    // Helper: get token from registry
    ...
    w.l("let tok = __nex_get_token(id);");
    w.l("tok.cancel();");
    w.ind = w.ind.saturating_sub(1);
    w.l("}");
    w.ind = w.ind.saturating_sub(1);
    ...
    // Join subtree (best-effort)
    w.l("fn __nex_join_subtree(_root: __NexTaskId) {");
    w.ind += 1;
    w.l("// NOTE: In this minimal runtime, we only join the handle we directly hold (structured join via __NexTask).");
    w.l("// Full descendant handle tracking is future work; we still guarantee cancellation propagation + registry pruning.");
    w.ind = w.ind.saturating_sub(1);
    w.l("}");
    w.l("");

    // cancelled()
    w.l("fn cancelled() -> bool {");
    w.ind += 1;
    w.l("__nex_get_current_token().is_cancelled()");
    w.ind = w.ind.saturating_sub(1);
    w.l("}");
    w.l("");

    // join(task)
    w.l("fn join(t: __NexTask) {");
    w.ind += 1;
    w.l("if let Some(h) = t.handle { let _ = h.join(); }");
    w.ind = w.ind.saturating_sub(1);
    w.l("}");
    w.l("");

    // cancel(task) => subtree cancel
    w.l("fn cancel(t: &__NexTask) {");
    w.ind += 1;
    w.l("__nex_cancel_subtree(t.id);");
    w.ind = w.ind.saturating_sub(1);
    w.l("}");
    w.l("");
    ...
    w.l("Err(_) => {");
    w.ind += 1;
    w.l("// Even on panic, attempt best-effort cleanup of remaining tasks");
...
fn emit_stmt(w: &mut W, s: &Stmt) {
    match s {
        Stmt::Let { name, ty, value } => {
            if let Some(t) = ty {
                w.l(&format!("let mut {}: {} = {};", name, rust_type(t), emit_expr(value)));
            } else {
        ...
        Stmt::Defer(b) => {
            // Minimal: emit block immediately (true defer requires scope stack)
            w.l("{");
            w.ind += 1;
            for st in &b.stmts {
                emit_stmt(w, st);
...
fn emit_expr(e: &Expr) -> String {
    match e {
        ...
        Expr::If { cond, then_block, else_block } => {
            ...
            out.push_str("{ ");
            for st in &then_block.stmts {
                // expressions in blocks: emit as statements
                // (minimal; future: expression blocks)
